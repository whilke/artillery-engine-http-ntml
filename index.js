// This is mostly a copy of the base Artillery.io http engine with support of NTML
 ...
 * file, You can obtain one at http://mozilla.org/MPL/2.0/. */
 ...
 function HttpNtmlEngine(script, ee, engineUtil) {
   this.engineUtil = engineUtil;
   this.template = engineUtil.template;
 ...
 HttpNtmlEngine.prototype.createScenario = function(scenarioSpec, ee) {
   var self = this;
 ...
 HttpNtmlEngine.prototype.step = function step(requestSpec, ee, opts) {
 
   opts = opts || {};
   let self = this;
   let config = this.config;
   let engineUtil = this.engineUtil;
   ...
   let f = function(context, callback) {
     ...
     async.eachSeries(
       ...
       function done(err) {
         ...
         let authConfigured = config.http != undefined && config.http.auth != undefined;
         let defaultConfig = {
             username :"",
             password :"",
             domain :"",
         ...
         let authConfig = authConfigured && config.http.auth ? config.http.auth : defaultConfig; 
         authConfig.type = authConfig.type || "NTML"
         let skipend = false;
 
         function handleType2Message(res, rp, cb, startedAt) {
           ...
           let header = res.headers['www-authenticate'];
           
           if (authConfig.type.toLowerCase() == "negotiate") {
             header = header.replace("Negotiate", "NTLM"); 
           }
           
           let type2msg = ntlm.parseType2Message(header);
           let type3msg = ntlm.createType3Message(type2msg, options);
 
           if (authConfig.type.toLowerCase() == "negotiate") {
             type3msg = type3msg.replace("NTLM", "Negotiate"); 
           }
 
           
           requestParams.headers.authorization = type3msg.replace("NTLM", "Negotiate");
           request(rp, cb)
         ...
         function handleType1Message(rp, cb, startedAt) {
           skipend = true;
           let options = {
             username: authConfig.username,
             password: authConfig.password,
           ...
           if (authConfig.type.toLowerCase() == "negotiate") {
             type1msg = type1msg.replace("NTLM", "Negotiate"); 
           }
           rp.headers.authorization = type1msg;
 
           request(rp, callback)
           .on("request", function(req){
             req.on('response', function(res) {
               handleType2Message(res, rp, cb, startedAt);
             });
           }).on('end', function(){}).on('error', function(err){
             debug(err);
         ...
         request(requestParams, maybeCallback)
           .on('request', function(req) {
             ...
             req.on('response', function updateLatency(res) {
               ...
               if (res.statusCode == 401 && res.headers['www-authenticate'] && authConfigured){
                   handleType1Message(requestParams, maybeCallback, startedAt);
               }
               else{
                 let code = res.statusCode;
                 const endedAt = process.hrtime(startedAt);
 ...
 HttpNtmlEngine.prototype.compile = function compile(tasks, scenarioSpec, ee) {
   let self = this;
